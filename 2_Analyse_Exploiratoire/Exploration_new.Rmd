...
regression linéaire sur le graphique précédent
```{r}
...
# Application methodes de classification non-suppervisée : 

K-MEANS
```{r}
data_macro_genre_quanti = data_games_macro[,c(5:6, 9, 11:13)]

data_macro_genre_kmeans = KMEANS(data_macro_genre_quanti, k=6)
pseudoF(data_macro_genre_kmeans)
kmeans.getk(data_macro_genre_quanti)
```
recuperation des données du premier cluster -> choses intérressantes ?
```{r}
clust1=as.vector(which(data_macro_genre_kmeans$cluster == 1))
#View(clust1)
data_macro_genre_clust1 = data_games_macro[clust1,]
```
...
# ACP
```{r}
data_macro_genre.pca = PCA(data_games_macro[,c(6, 9:13)])
plot(data_macro_genre.pca, type = "cor")
plot(data_macro_genre.pca)
data_macro_genre.pca$eig
```
K-MEANS sur ACP
```{r}
data_macro_genre_kmeans = KMEANS(data_macro_genre.pca$ind$coord, k=2)
pseudoF(data_macro_genre_kmeans)
kmeans.getk(data_macro_genre.pca$ind$coor)
```
recuperation des données du premier cluster -> choses intérressantes ?
```{r}
clust1=as.vector(which(data_macro_genre_kmeans$cluster == 1))
